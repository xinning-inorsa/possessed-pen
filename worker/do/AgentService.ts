import { AmazonBedrockAnthropicProvider } from '@ai-sdk/amazon-bedrock/anthropic'
...
import { createBedrockAnthropicProvider, getBedrockModelId } from '../bedrock'
...
export class AgentService {
	...
	bedrock: AmazonBedrockAnthropicProvider
	...
	constructor(env: Environment) {
		...
		this.bedrock = createBedrockAnthropicProvider(env)
	}

	getModel(modelName: AgentModelName): LanguageModel {
		const modelDefinition = getAgentModelDefinition(modelName)
		if (modelDefinition.provider === 'bedrock') {
			return this.bedrock(getBedrockModelId(this.env)) as LanguageModel
		}
	...
	switch (definition.provider) {
		...
		case 'bedrock':
			return {
				bedrock: {
					reasoningConfig: {
						type: definition.thinking === 'adaptive' ? 'adaptive' : 'disabled',
						...(definition.effort ? { maxReasoningEffort: definition.effort } : {}),
					},
				},
			}
	}
